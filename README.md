...
- 句子按各自分数着色，不绘制段落线框；图注匹配同页上方实际图片/矢量区域，图片与图注一起高亮。Figure 候选默认获得轻微排序加权，问题明确提到图、Figure、chart、plot 或 diagram 时进一步加权。图片判断仅依据图注；复杂多子图、跨页图注、无编号图注可能漏关联，不声称看懂图内数据。
